...
import { createGeneration, consumeCreditAtomic, ensureUsage, getActiveSubscription, getAdminStats, getGenerationById, getTemplates, getTemplateById, deleteTemplate, getUsage, getUserGenerations, updateGeneration, joinSubscriberWaitlist } from "./db";
...
const quotaWindowKey = () => "rolling-24h";
...
export const appRouter = router({
  ...
  usage: router({
    current: protectedProcedure.query(async ({ ctx }) => { const subscription = await getEffectiveSubscription(ctx.user, ctx.supabaseAccessToken); const plan = (subscription?.plan ?? "FREE") as Plan; const limit = PLAN_LIMITS[plan]; const windowKey = quotaWindowKey(); const row = await ensureUsage(ctx.user.id, windowKey, limit); return { used: row?.generationsUsed ?? 0, limit, plan, resetAt: row?.resetAt?.toISOString() ?? null }; }),
  }),
  generations: router({
