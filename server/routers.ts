...
export const appRouter = router({
  ...
  usage: router({
    current: protectedProcedure.query(async ({ ctx }) => { const subscription = await getActiveSubscription(ctx.user.id); const plan = subscription?.plan ?? "FREE"; const limit = PLAN_LIMITS[plan]; const month = monthKey(); const row = await ensureUsage(ctx.user.id, month, limit); return { used: row?.generationsUsed ?? 0, limit, plan }; }),
  }),
  generations: router({
