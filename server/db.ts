...
import { InsertUser, User, users, generations, templates, subscriptions, usage, Generation, Template } from "../drizzle/schema";
...
export function normalizeGenerationMedia(row: Generation) { const proxyUrl = (key: string | null | undefined) => key ? `/manus-storage/${key.split("/").map(encodeURIComponent).join("/")}` : undefined; return { ...row, originalImageUrl: proxyUrl(row.originalImageKey) ?? row.originalImageUrl, generatedImageUrl: proxyUrl(row.generatedImageKey) ?? row.generatedImageUrl }; }
...
export async function ensureUsage(userId: number, month: string, limit: number) { const db = await getDb(); if (!db) return undefined; await db.insert(usage).values({ userId, month, generationLimit: limit }).onDuplicateKeyUpdate({ set: { generationLimit: limit } }); return getUsage(userId, month); }
...
export async function consumeCreditAtomic(userId: number, month: string) { const db = await getDb(); if (!db) throw new Error("Base de données indisponible"); const updated = await db.update(usage).set({ generationsUsed: sql`${usage.generationsUsed} + 1` }).where(and(eq(usage.userId, userId), eq(usage.month, month), sql`${usage.generationsUsed} < ${usage.generationLimit}`)); return updated[0].affectedRows === 1; }
